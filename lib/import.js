...
Import.prototype = {
	...
	importPosts: function(next) {
		...
		async.eachLimit(this.mem._pids, this.config.nbb.postsBatchSize, function(_pid, done) {
			...
			logger.debug('Importing ' + _self.mem._pids.length + ' posts took: ' + ((+new Date() - startTime)/1000).toFixed(2) + ' seconds');
			next();
		});
	},
	...
	_runNbbSetup: function(next) {
		var node,
			...
			setup = function(next) {
				...
				if (true || result.stdout.indexOf('NodeBB Setup Completed') > -1) {
