...
// I'm lazy, dont hire me
	...
	// using a temporary manual for till https://github.com/neocotic/html.md/pull/43 gets merged
...
Import.prototype = {
	...
	importUsers: function(next) {
		var count = 0,
			...
			nbbRegisteredUsersGid = storage.getItem('nbb.groups.registered-users.gid'),
			startTime = +new Date(),
			flushed = this.config.nbb.setup.runFlush;
		...
		async.eachLimit(this.mem._tids, this.config.nbb.topicsBatchSize, function(_tid, done) {
			...
			if (importedTopic || skippedTopic) {
				...
				if (!importedUser || !importedCategory) {
					...
					Topics.post(importedUser.uid, normalizedTopic._title, _self.convert(normalizedTopic._content), importedCategory.cid, function(err, returnTopic){
						if (err) {
		...
		async.eachLimit(this.mem._pids, this.config.nbb.postsBatchSize, function(_pid, done) {
			...
			if (importedPost || skippedPost) {
				...
				if (!importedUser || !importedTopic) {
					...
					Posts.create(importedUser.uid, importedTopic.tid, _self.convert(normalizedPost._content || ''), function(err, postReturn){
						if (err) {
		...
		async.parallel([
			function(done){
				Group.getGidFromName('administrators', function(err, gid) {
					...
					storage.setItem('nbb.groups.administrators.gid', gid, done);
				});
			},
			function(done){
				// todo: I don't need this anymore, User.create does that automatically
				Group.getGidFromName('registered-users', function(err, gid) {
					if (err) {
						_self.logger.error(err);
						done(err);
					}
					storage.setItem('nbb.groups.registered-users.gid', gid, done);
				});
			}
