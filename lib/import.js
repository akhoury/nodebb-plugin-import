...
// a quick logger
	...
	Import = function (config) {
		...
		// same for line 5
		if (config.nbb.setup.setupVal.database === 'redis') {
			try { DB = module.parent.require('./db.js');}
			catch (e) { DB = require('../../../src/database/redis.js');}
		} else if (config.nbb.setup.setupVal.database === 'mongo') {
			try { DB = module.parent.require('./db.js');}
			catch (e) { DB = require('../../../src/database/mongo.js');}
		} else {
...
Import.prototype = {
	...
	importUsers: function(next) {
		...
		async.eachLimit(this.mem._uids, this.config.nbb.usersBatchSize, function(_uid, done) {
			...
			var storedUser = storage.getItem('u.' + _uid),
				...
				skippedUser = storedUser.skipped;
			...
			logger.debug('Importing ' + _self.mem._uids.length + ' users took: ' + ((new Date().getTime()-startTime)/1000).toFixed(2) + ' seconds');
	...
	importTopics: function() {
		...
		async.eachLimit(this.mem._tids, this.config.nbb.topicsBatchSize, function(_tid, done) {
			...
			var skippedTopic = storedTopic.skipped;
			...
			logger.debug('Importing ' + _self.mem._tids.length + ' topics took: ' + ((new Date().getTime()-startTime)/1000).toFixed(2) + ' seconds');
			next();
		});
	},
	...
	importPosts: function() {
		...
		async.eachLimit(this.mem._pids, this.config.nbb.postsBatchSize, function(_pid, done) {
			...
			var storedPost = storage.getItem('p.' + _pid),
				...
				skippedPost = storedPost.skipped;
			...
			logger.debug('Importing ' + this.mem._pids.length + ' posts took: ' + ((new Date().getTime()-startTime)/1000).toFixed(2) + ' seconds');
			next();
		});
	},
