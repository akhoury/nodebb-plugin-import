...
Import.prototype = {
	...
	setup: function(next) {
		...
		// temp memory
		this.logger.info('Loading storage dir:' + this.config.storageDir, + ' files into memory, please be patient ... ');
		this.mem = {
	...
	importCategories: function(next) {
		...
		async.eachSeries(this.mem._cids, function(_cid, done) {
			count++;
	...
	importUsers: function(next) {
		...
		async.eachSeries(this.mem._uids, function(_uid, done) {
			count++;
				...
				if (!user.username) {
					...
					User.create(user.username, user.password, user.email, function(err, uid) {
						...
						if (err) {
							...
							logger.log('[user-json] {"email":"' + user.email + '","username":"' + user.username + '","pwd":"' + user.password + '",_uid":' + _uid + ',"uid":' + uid +',"ms":' + fields.joindate + '},');
							logger.log('[user-csv] ' + user.email + ',' + user.username + ',' + user.password + ',' + _uid + ',' + uid + ',' + fields.joindate);
	...
	importTopics: function(next) {
		var _self = this,
			...
			startTime = +new Date(),
			flushed = this.config.nbb.setup.runFlush;
		async.eachSeries(this.mem._tids, function(_tid, done) {
			count++;
	...
	importPosts: function(next) {
		...
		async.eachSeries(this.mem._pids, function(_pid, done) {
			count++;

			var storedPost = storage.getItem('p.' + _pid);
			if (!storedPost || !storedPost.normalized) {
				logger.warn('[c:' + count + '] normalized post:_pid: ' + _pid + ' it doesn\'t exist in storage');
	...
	report: function(next) {
		...
		logger.log('----> Or if you saved these stdout logs, '
